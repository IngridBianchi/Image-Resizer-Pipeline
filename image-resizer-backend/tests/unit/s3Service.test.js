...
describe("S3Service", () => {
  ...
  test("getMetadata debe devolver metadata del objeto", async () => {
    mockS3.send.mockResolvedValueOnce({
      Metadata: { width: "800", height: "600" }
    });

    const result = await s3Service.getMetadata("bucket", "key");
    expect(result).toEqual({ width: "800", height: "600" });
    expect(mockS3.send.mock.calls[0][0]).toBeInstanceOf(HeadObjectCommand);
  });

  test("getMetadata debe devolver objeto vacío si el objeto no tiene metadata", async () => {
    mockS3.send.mockResolvedValueOnce({});
