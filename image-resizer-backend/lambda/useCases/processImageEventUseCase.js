...
class ProcessImageEventUseCase {
  ...
  async execute(event) {
    ...
    const isS3Event = event && Array.isArray(event.Records) && event.Records.length > 0;
    const hasDimensionsInEvent = event?.width !== undefined || event?.height !== undefined;

    // Para invocaciones directas o con dimensiones explícitas, validar antes de I/O
    let directWidth, directHeight;
    if (!isS3Event || hasDimensionsInEvent) {
      directWidth = parseDimension(event?.width, DEFAULT_DIMENSION);
      directHeight = parseDimension(event?.height, DEFAULT_DIMENSION);
    }
    ...
    for (const item of records) {
      ...
      if (isS3Event && !hasDimensionsInEvent) {
        // Flujo S3-driven: extraer dimensiones de la clave.
        // Formato esperado: {uuid}/{W}x{H}/{filename}  (generado por presign-server)
        const segments = item.key.split('/');
        const dimMatch = segments.length >= 2 ? segments[1].match(/^(\d+)x(\d+)$/) : null;
        width = parseDimension(dimMatch?.[1], DEFAULT_DIMENSION);
        height = parseDimension(dimMatch?.[2], DEFAULT_DIMENSION);
      } else {
        width = directWidth;
        height = directHeight;
      }
